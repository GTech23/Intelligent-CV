...
const EducationForm = ({ editIndex }) => {
  const { formData, setFormData } = useResume();
  const navigate = useNavigate();
  const education = formData.education || [];
  const index =
    editIndex !== undefined ? Number(editIndex) : education.length - 1;
  const edu = education[index] || {};
  const savedEdu = useRef(false);
  ...
  const handleBack = () => {
    if (!savedEdu.current) {
      const isEmpty = Object.values(edu).every((v) => !v);
      if (isEmpty && education.length > 0) {
